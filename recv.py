...
FF_PLAY = '{ffplay} -fs -an -sn -fflags nobuffer+nofillin+fastseek -flags low_delay -infbuf -probesize 32 -analyzeduration 0 -framedrop -sync ext -video_size {xres}x{yres} -pixel_format {pix_fmt} -f rawvideo -i pipe:'
...
def play(options: Options) -> None:
    ...
    with Finder() as finder:
        ...
        try:
            ...
            proc = subprocess.Popen(shlex.split(cmd_str), stdin=subprocess.PIPE, bufsize=0)
